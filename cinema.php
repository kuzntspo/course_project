    ...
    <?php
        ...
        $district=$_POST['district'];
            ...
            if(empty($adm)&empty($district)){
                $sql = "SELECT Name,Location,Photo FROM `ATTRACTION` WHERE Name RLIKE 'КИНОТЕАТР' || Name RLIKE 'CINEMA' || Name RLIKE 'ВИРТУАЛЬНЫЙ' || Name RLIKE 'ВИРТУАЛЬНАЯ'";
                $result = mysqli_query($connect, $sql);//обращение к бд
            }
            else{
                $sql = "SELECT Name,Location,Photo FROM `ATTRACTION` WHERE (Name RLIKE 'КИНОТЕАТР' || Name RLIKE 'CINEMA' || Name RLIKE 'ВИРТУАЛЬНЫЙ' || Name RLIKE 'ВИРТУАЛЬНАЯ') AND (AdmArea RLIKE '$adm'|| District RLIKE '$district')";
                $result = mysqli_query($connect, $sql);//обращение к бд
            }
