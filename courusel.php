    ...
    <?php
        ...
        $district=$_POST['district'];
            ...
            if(empty($adm)&empty($district)){
                ...
                $sql = "SELECT Name,Location,Photo FROM `ATTRACTION` WHERE 
                (Name RLIKE 'КАРУСЕЛЬ' || Name RLIKE 'CAROUSEL'|| Name RLIKE 'JET'|| Name RLIKE 'ДЖЕТ'|| Name RLIKE 'MERRY' || Name RLIKE 'SWING'|| Name RLIKE 'AIR RACE' || Name RLIKE 'RED BARON' || Name RLIKE 'AROUND' || Name RLIKE 'РАКУШКИ'|| 
                Name IN('ВОЗДУШНЫЙ ШАР','ВЕРТОЛЕТЫ','MAGIC BIKES','ОСЬМИНОГ', 'ОРБИТА-420', 'ПИРАТ', 'РАЛЛИ', 'ДРАКОНЫ', 'МИНИ- ПОЛИП', 'АТТРАКЦИОН СЕРИИ 10MEH ФУТУРИСТИЧЕСКАЯ', 'КЕГУРУ','RED BARON INTERACTIVE','FLYING TIGERS', 'ЛЕТАЮЩИЕ СЛОНЫ','CHOCO CUPS', 'FLYING BIKES', 'BOUNCE SPIN', 'ЭВРИКА')) AND (AdmArea RLIKE '$adm'|| District RLIKE '$district') ";
                $result = mysqli_query($connect, $sql);//обращение к бд
            }
