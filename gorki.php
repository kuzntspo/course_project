    ...
    <?php
        ...
        include "db_connect.php";
            if(empty($adm)&empty($district)){
                ...
                $sql = "SELECT Name,Location,Photo FROM `ATTRACTION` WHERE (Name RLIKE 'ЗИМНЯЯ ГОРКА'|| Name RLIKE 'ГОРКА ДЛЯ КАТАНИЯ'|| Name RLIKE 'ГОРКА ДЕРЕВЯННАЯ') AND (AdmArea RLIKE '$adm'|| District RLIKE '$district')";
                $result = mysqli_query($connect, $sql);//обращение к бд
            }
