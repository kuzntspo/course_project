...
<body>
    ...
    <div class="dropdown">
            <button class="dropbtn">В аквапарках</button>
    ...
    $district=$_POST['district'];
        ...
        if(empty($adm)&empty($district)){
            $sql = "SELECT Name,Location,Photo FROM `ATTRACTION_2` WHERE Location RLIKE 'АКВА'";
            $result = mysqli_query($connect, $sql);//обращение к бд
        }
        else{
            $sql = "SELECT Name,Location,Photo FROM `ATTRACTION_2` WHERE Location RLIKE 'АКВА' AND (AdmArea RLIKE '$adm'|| District RLIKE '$district')";
            $result = mysqli_query($connect, $sql);//обращение к бд
        }
