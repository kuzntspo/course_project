    ...
    <?php
        ...
        include "db_connect.php";
            if(empty($adm)&empty($district)){
                ...
                $sql = "SELECT Name,Location,Photo FROM `ATTRACTION` WHERE (Name RLIKE 'БАТУТ' || Name RLIKE 'НАДУВНАЯ' || Name RLIKE 'БАТУТНЫЙ' || Name RLIKE 'СПОРТ'|| 
                Name IN('ТИГРИНАЯ СТРАНА - 2','ББ.3001','ЦАРЬ ГОРЫ','КИТ', 'ПОДВОДНЫЙ МИР 2', 'ВУЛКАН', 'ОСТРОВ СОКРОВИЩ', 'МГ.4002', 'ДЖУНГЛИ', 'КОСМИК', 'ГАЛЕОН','ВИНИ-ПУХ','ЗООАРЕНА', 'МАДАГАСКАР','СЛАСТЕНА', 'МГ.4049', 'ВОЛНА', 'ПОДВОДНЫЙ МИР')) AND (AdmArea RLIKE '$adm'|| District RLIKE '$district')";
                $result = mysqli_query($connect, $sql);//обращение к бд
            }
